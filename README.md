...
- **Antigravity Remote Control Integration**: Every running task automatically captures or synthesizes its Remote Control session URL (`https://antigravity.google.com/c/<conversation_id>?instance=<instance>`). Live links are surfaced in GitHub PR comments, the REST API, MCP tools, and the TUI dashboard.
