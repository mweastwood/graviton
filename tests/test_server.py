...
class TestGravitonHandler(unittest.TestCase):
    ...
    def test_main_accepts_quota_endpoint_cli_arg(
        ...
        custom_endpoint = "https://custom-daily.googleapis.com/v1internal:retrieveUserQuotaSummary"
        with patch.dict("os.environ", {"ANTIGRAVITY_QUOTA_ENDPOINT": ""}):
            with patch("sys.argv", ["graviton-server.py", "--quota-endpoint", custom_endpoint]):
                server_mod.main()
    ...
    def test_main_accepts_quota_endpoint_env_var(
        ...
        custom_endpoint = "https://custom-env.googleapis.com/v1internal:retrieveUserQuotaSummary"
        with patch.dict("os.environ", {"ANTIGRAVITY_QUOTA_ENDPOINT": custom_endpoint}):
            with patch("sys.argv", ["graviton-server.py"]):
                server_mod.main()
    ...
    def test_main_accepts_skills_dir_cli_arg(
        ...
        custom_skills = "/tmp/custom_skills"
        with patch("sys.argv", ["graviton-server.py", "--skills-dir", custom_skills]):
            server_mod.main()

        _, kwargs = mock_tm.call_args
        self.assertEqual(kwargs.get("skills_dir"), custom_skills)
    ...
    def test_graceful_shutdown_persists_model_selection_state(self):
        ...
        mock_qt.stop_background_polling.assert_called_once()
    ...
    def test_main_starts_quota_background_polling_custom_interval(
        ...
        with patch("sys.argv", ["graviton-server.py", "--quota-poll-interval", "10.0"]):
            server_mod.main()
    ...
    def test_main_starts_quota_background_polling_fallback_non_positive_interval(
        ...
        with patch("sys.argv", ["graviton-server.py", "--quota-poll-interval", "0.0"]):
            server_mod.main()
    ...
    def test_main_starts_quota_background_polling_fallback_non_finite_interval(
        ...
        with patch("sys.argv", ["graviton-server.py", "--quota-poll-interval", "inf"]):
            server_mod.main()
    ...
    def test_main_starts_quota_background_polling_graviton_env_var(
        ...
        with patch.dict(os.environ, {"GRAVITON_QUOTA_POLL_INTERVAL": "12.5"}), patch("sys.argv", ["graviton-server.py"]):
            server_mod.main()
    ...
    def test_main_disables_quota_background_polling_with_flag(
        ...
        with patch("sys.argv", ["graviton-server.py", "--no-quota-background-polling"]):
            server_mod.main()
    ...
    def test_main_disables_quota_background_polling_with_env_var(
        ...
        with patch.dict(os.environ, {"GRAVITON_QUOTA_BACKGROUND_POLLING": "false"}), patch("sys.argv", ["graviton-server.py"]):
            server_mod.main()
    ...
    def test_main_disables_quota_background_polling_with_quota_env_var(
        ...
        env = {"QUOTA_BACKGROUND_POLLING": "false"}
        with patch.dict(os.environ, env, clear=False), patch("sys.argv", ["graviton-server.py"]):
            os.environ.pop("GRAVITON_QUOTA_BACKGROUND_POLLING", None)
            server_mod.main()
    ...
    def test_main_quota_poll_interval_invalid_fallback_warning(
        ...
        with patch("graviton_server.logger.warning") as mock_warn, patch("sys.argv", ["graviton-server.py", "--quota-poll-interval", "0"]):
            server_mod.main()

        mock_qt_inst.start_background_polling.assert_called_once_with(poll_interval=5.0)
        self.assertTrue(
            any("Invalid quota_poll_interval" in str(c) for c in mock_warn.call_args_list)
        )
...
class TestGravitonServerSupervisorPipeline(unittest.TestCase):
    """Unit tests for supervisor CLI flags and event routing in bin/graviton-server.py."""
    ...
    def test_do_post_with_task_manager_supervisor_mode(self):
        mock_tm = MagicMock()
        mock_tm.use_supervisor = True
        payload_dict = {"action": "opened", "number": 19}
    ...
    def test_do_post_direct_execution_with_supervisor_active(self, mock_thread_cls, mock_run_goal):
        payload = json.dumps({"action": "opened", "number": 23}).encode("utf-8")
        handler = MagicMock(spec=GravitonHandler)
        handler.headers = {
            "Content-Length": str(len(payload)),
            "X-GitHub-Event": "pull_request",
        ...
        goal_calls = [c for c in mock_thread_cls.call_args_list if c[1].get("target") == mock_run_goal]
        self.assertEqual(len(goal_calls), 1)
        self.assertEqual(goal_calls[0][1]["kwargs"]["agent_name"], "code_reviewer")
