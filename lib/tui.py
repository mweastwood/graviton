...
class TerminalDashboard:
    ...
    @property
    def _old_term_settings(self) -> Optional[Any]:
        if hasattr(self, "_input_listener") and self._input_listener is not None:
            return self._input_listener._old_term_settings
        return getattr(self, "_stored_old_term_settings", None)

    @_old_term_settings.setter
    def _old_term_settings(self, value: Optional[Any]) -> None:
        self._stored_old_term_settings = value
        if hasattr(self, "_input_listener") and self._input_listener is not None:
    ...
    def stop(self):
        ...
        self._running = False
        if hasattr(self, "_input_listener") and self._input_listener:
    ...
    def _is_incomplete_escape_sequence(raw_bytes: bytes) -> bool:
        ...
        return is_incomplete_escape_sequence(raw_bytes)
    ...
    def _split_incomplete_utf8_tail(raw_bytes: bytes) -> tuple[bytes, bytes]:
        ...
        return split_incomplete_utf8_tail(raw_bytes)
    ...
    def _stdin_loop(self):
        ...
        self._input_listener.run_loop(is_running_func=lambda: self._running)
    ...
    def _restore_termios(self):
        ...
        if hasattr(self, "_input_listener") and self._input_listener is not None:
            self._input_listener.restore_terminal()
        elif self._old_term_settings is not None and HAS_TERMIOS:
            try:
                if sys.stdin.isatty():
                    fd = sys.stdin.fileno()
